...
See [Configuring a Change Discovery API Extraction job](#configuring-a-change-discovery-api-extraction-job)
...
`IIIFMetaDataExtractor` reads the `.iiif` metadata files generated by Cineast when it is downloading your IIIF resources. This metadata is stored to the database you configured and allows [vitrivr-ng](https://github.com/vitrivr/vitrivr-ng) to fetch IIIF resources directly from their parent IIIF servers without using the local copy that Cineast may or may not have.
...
A Change Discovery API job can be configured in the `orderedCollectionUrl` variable in the `iiif` object block of the configuration file. Here is an extract from the [example configuration file](https://github.com/vitrivr/cineast/wiki/iiif_extraction_config.json). 
